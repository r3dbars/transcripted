...
├── AGENTS.md                ← Initialization order and boot sequence documentation
...
**Each subfolder in Sources/ has its own AGENTS.md with component-specific knowledge. Read the relevant AGENTS.md before modifying any file in that folder.**
...
- **Read the relevant AGENTS.md** in the engine's `Sources/` subfolder for component-specific knowledge
...
2. **`Sources/Draft/AGENTS.md`** — Add to the bundle ID mapping table
...
4. **`Sources/Style/AGENTS.md`** — Update the file format example
...
4. **`Sources/Capture/AGENTS.md`** — Update CapturedContext struct docs
...
4. **`Sources/UI/AGENTS.md`** — Document any new states, views, or behaviors
...
5. **`Sources/UI/AGENTS.md`** — Document any new sections or layout changes
...
A `/push` slash command is available (`.Codex/commands/push.md`) that handles the full flow: auth switch → stage → commit → push.
...
- **Native analysis engine** — Replaced the Python subprocess with Swift-native `AnalysisEngine` using DispatchSource file watching, eliminating subprocess crashes, cold start latency, and port conflicts. See `Sources/Analysis/AGENTS.md`.
...
> Each Sources/ subfolder has its own AGENTS.md with component-specific details. Items marked "→ See ..." are summarized here but documented in depth in the subfolder. Cross-cutting items that span multiple components are kept in full.

### Audio & Speech

- **Apple Speech buffer resets are undocumented** — → See `Sources/Speech/AGENTS.md` § "Critical Gotchas"
- **Pro audio interfaces break SFSpeechRecognizer** — USB interfaces like BEACN Mic (96kHz/4ch) cause error 1110. Fix: force mono tap format. → See `Sources/Speech/AGENTS.md` § "Critical Gotchas"
- **Microphone permission can be revoked at runtime** — Check `AVCaptureDevice.authorizationStatus` before `installTap()`. → See `Sources/Speech/AGENTS.md` § "Critical Gotchas"
...
- **MLXEngine is a Swift actor** — wraps mlx-swift-lm for thread-safe model access. See `Sources/Local/AGENTS.md`
...
- **Global hotkey timing is critical** — screenshot AND frontmost app reference must be captured synchronously in the C callback before any `Task { @MainActor }` dispatch, or window focus shifts and you capture Draft instead of the target app. → See `Sources/Capture/AGENTS.md`
...
- **Hotkey labels in ContextCaptureEngine** — ⌥D = hotkey ID 1 (draft mode), ⌥Space = hotkey ID 2 (dictation mode). Both use signature `0x44524654` ('DRFT'). → See `Sources/Capture/AGENTS.md`
...
- **iMessage `text` stores U+FFFC for attachment-only messages** — Filter by `trimmed.count < 2`, not word count. → See `Sources/Messages/AGENTS.md`
- **Prompts are externalized to JSON** — `PromptStore` reads `prompts.json` on launch. `DefaultPrompts` enum holds hardcoded fallbacks. → See `Sources/Prompts/AGENTS.md`
