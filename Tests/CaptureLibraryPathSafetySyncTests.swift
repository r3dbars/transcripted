...
/// CaptureLibraryPathSafety.swift is a SYNCED COPY that exists byte-identically
/// in three places, one per build unit that cannot share a module boundary with
/// the others (see that file's header comment for why real duplicated files
/// were chosen over a git symlink — a checkout with `core.symlinks=false`
/// materializes a symlink as a text file containing the literal link target,
/// which fails to compile). This test is the enforcement mechanism: if any one
/// copy diverges from the others by even a byte, this fails and names exactly
/// which paths need to be brought back in sync.
