...
func testNightlySecurityContract() {
    ...
    runSuite("Nightly security checker exposes strict release-health flags") {
        ...
        assertTrue(preflight.contains("--github-release-json Tests/Fixtures/release-health-github-release-1.1.49.json"), "agent preflight should suggest a strict checker command with deterministic GitHub release metadata")
        assertTrue(matrix.contains("--github-release-json Tests/Fixtures/release-health-github-release-1.1.49.json"), "test matrix should suggest a strict checker command with deterministic GitHub release metadata")
    }
    ...
    runSuite("Nightly security checker fails stale GitHub release asset metadata") {
        let passing = runNightlySecurityChecker(arguments: [
            ...
            "--github-release-json", "Tests/Fixtures/release-health-github-release-1.1.49.json"
        ])
