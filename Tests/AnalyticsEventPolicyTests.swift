...
func testAnalyticsEventPolicy() {
    ...
    runSuite("AnalyticsEventPolicy preserves dictation auto-send attribution") {
        ...
        assertEqual(dictationCompleted?.allowedProperties.contains("failure_kind"), true, "dictation completion should preserve a normalized terminal paste failure kind")
    ...
    runSuite("AnalyticsEventPolicy keeps the dictation attempt denominator whole") {
        ...
        assertEqual(
            ...
            "dropping a field the success event has would make the two uncomparable in a funnel; start latency is the one field a request cannot know yet"
        )
        assertTrue(
    ...
    runSuite("AnalyticsEventPolicy meeting_recording_stopped system_stream_present key is not silently filtered") {
        ...
        assertEqual(policy?.allowedProperties.contains("capture_outcome"), true, "meeting stop should expose a typed terminal capture outcome")
        assertEqual(healthPolicy?.allowedProperties.contains("capture_outcome"), true, "meeting health snapshots should expose the same typed terminal capture outcome")
