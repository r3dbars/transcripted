...
func testCaptureUndo() async {
    ...
    await runSuite("CaptureUndoManager.deleteRange — rewrites immediately, undo restores the original content verbatim") {
        let fm = FileManager.default
        let root = temporaryCaptureUndoTestRoot()
        try? fm.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: root) }

        let dayFile = root.appendingPathComponent("Dictations_2026-08-06.md")
        let original = """
        # Dictations for August 6, 2026
        ...
        first body
        """
        try? original.write(to: dayFile, atomically: true, encoding: .utf8)

        let manager = await CaptureUndoManager()
        let id = "dictation-entry-1"

        do {
            _ = try await manager.deleteRange(
                id: id,
                in: dayFile,
                originalContent: original,
                newContent: rewritten,
                message: CaptureUndoMessage.deleted("Second")
            )

            let afterDelete = try String(contentsOf: dayFile, encoding: .utf8)
            assertEqual(afterDelete, rewritten, "file should be rewritten immediately without the deleted entry")

            await manager.undo(id)

            let afterUndo = try String(contentsOf: dayFile, encoding: .utf8)
            assertEqual(afterUndo, original, "undo should restore the exact original content")
        } catch {
            assertTrue(false, "deleteRange should not throw: \(error)")
        }
    }

    await runSuite("CaptureUndoManager.deleteRange — finalize leaves the rewritten content in place") {
        let fm = FileManager.default
        let root = temporaryCaptureUndoTestRoot()
        try? fm.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: root) }
        ...
        let dayFile = root.appendingPathComponent("Dictations_2026-08-06.md")
        let original = "## 9:00 AM - First\n\nfirst body\n\n## 9:05 AM - Second\n\nsecond body\n"
        let rewritten = "## 9:00 AM - First\n\nfirst body\n"
        try? original.write(to: dayFile, atomically: true, encoding: .utf8)

        let manager = await CaptureUndoManager()
        let id = "dictation-entry-2"
        ...
        do {
            ...
            let final = try String(contentsOf: dayFile, encoding: .utf8)
            assertEqual(final, rewritten, "finalize should leave the rewritten content in place")
            let pending = await manager.isPending(id)
            assertFalse(pending, "id should no longer be pending after finalize")
        } catch {
            assertTrue(false, "deleteRange should not throw: \(error)")
