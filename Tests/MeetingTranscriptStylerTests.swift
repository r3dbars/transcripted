...
private func testMeetingTranscriptStylerFailsClosedOnUnparseableBody() {
    ...
    Transcript text in a shape the entry parser does not understand.
    More lines that must survive the restyle pass untouched.
    """
    ...
    assertEqual(styled.url, transcriptURL, "Unparseable transcripts should keep their original path")
    assertEqual(updated, raw, "Unparseable transcript bodies must never be rewritten")
    assertFalse(
        updated?.contains("_No transcript captured._") == true,
        "Unparseable transcript bodies must not be replaced with the empty placeholder"
    )
}

private func testMeetingTranscriptStylerStillRewritesGenuinelyEmptyTranscript() {
    let directory = makeTemporaryTestDirectory()
    defer { try? FileManager.default.removeItem(at: directory) }
    ...
    *Generated by Transcripted with Parakeet + PyAnnote (local) | Duration: 0:12 | 0 words | 0 speakers*
    """
    try? raw.write(to: transcriptURL, atomically: true, encoding: .utf8)
    ...
    assertEqual(styled.title, "Quick notes", "Empty transcripts should still get the short-meeting title")
    assertTrue(
        updated.contains("_No transcript captured._"),
        "Genuinely empty transcripts should still be rewritten with the empty placeholder"
    )
}
