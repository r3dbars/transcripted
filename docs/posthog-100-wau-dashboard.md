...
| Local summary beta funnel | Are beta summaries discoverable, prepared, run, and useful? | `settings_page_viewed`, `settings_action_clicked`, `settings_toggle_changed`, `activation_artifact_action_clicked` with `action_kind = 'local_summary'`, plus `local_summary_requested`, `local_summary_finished`, `local_summary_failed`, and `local_summary_cancelled` | Legacy `local_meeting_summary_*` events may still appear during compatibility windows; keep model/provider/status/failure as enums only |
...
5. **Local summary beta funnel**
   - Funnel: Beta page viewed -> summary setting/action touched -> model
     prepared -> `activation_artifact_action_clicked` with
     `action_kind = 'local_summary'` -> `local_summary_requested` ->
     finished, failed, or cancelled -> summary opened through the same artifact-action
     event.
   - Keep model/provider/status/failure fields enum-only and keep generated
     summary content local.
...
with existing reviewed events. Local-summary request/open behavior should keep
using `activation_artifact_action_clicked` with `action_kind = 'local_summary'`
unless a future product question truly needs a separate lifecycle event.
