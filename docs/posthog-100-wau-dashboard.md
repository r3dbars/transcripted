...
- **Sourced agent-use proof:** `agent_capture_query_observed` grouped by
  `query_kind`, `artifact_kind`, `source_count_bucket`, `capture_age_bucket`,
  and `return_window_bucket`. This proves a saved local artifact was queried by
  MCP/agent tooling, not that the answer was good.
