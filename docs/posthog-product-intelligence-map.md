...
| Where does onboarding abandon? | `onboarding_shown`, `onboarding_step_viewed`, permission/model/CTA events, `permission_readiness_observed`, first dictation start/save/empty/stop, meeting dry run, agent CTA, reporting toggle, `onboarding_completed`, plus `workflow_abandoned`. | Mostly query shape. Readiness observations separate required microphone permission and meeting System Audio from completion; abandoned workflow fields answer confident terminal exits. | Existing: `step_id`, `step_index`, `flow_elapsed_bucket`, `step_elapsed_bucket`, `model_state`, `permission_kind`, `prior_status`, `to_status`, `cta`, `cta_type`, `first_dictation_saved`, `meeting_dry_run_completed`, `completion_flow`, `stage`, `reason_kind`; readiness: `required_microphone_ready`, `meeting_system_audio_ready`, `microphone_status`, `system_audio_status`, `source`. | `activation.ordered_funnel` plus an onboarding-friction query grouped by readiness, `step_id`, `stage`, `reason_kind`, and `flow_elapsed_bucket`. | `PermissionsOnboardingView.swift`; app launch/onboarding readiness observation; `ActivationTelemetry.trackWorkflowAbandoned` for confident exits. | `AnalyticsEventPolicyTests` pins onboarding funnel fields and sanitizer behavior. | Dashboard-only unless a specific abandoned-stage rollup proves materially easier to operate. |
| Are users forming post-save habits? | `activation_second_artifact_saved`, `activation_return_proxy_observed`, `agent_capture_query_observed`, `dictation_completed`, `dictation_artifact_saved`, `meeting_transcript_saved`. | Habit quality is still aggregate/proxy. There is no client-emitted active-days bucket; the retention script computes habit cohorts from PostHog event history. | Existing: `days_since_first_bucket`, `first_artifact_kind`, `second_artifact_kind`, `return_window_bucket`, `prior_artifact_kind`, `artifact_kind`, `client_family`, `tool_kind`, `capture_kind`, `source_count_bucket`, `result_count_bucket`. Future: `repeat_modality`, `active_days_bucket` only if computed without IDs/content. | `scripts/ops/retention-cohort-report.py`, `wau.active_devices_by_week`, `wau.daily_active_devices`, `activation.return_windows`. | `ActivationTelemetry` saved-artifact/return proxy; retention scripts for aggregate cohorts. | `Tests/RepoCommandContractTests.swift` keeps first-value events in health probes/docs; retention script has self-test. | Dashboard/script-only for now. Medium PR only if PostHog-side cohorts cannot answer repeat modality. |
| Are local summaries useful enough to keep? | `local_summary_requested`, `local_summary_finished`, `local_summary_failed`, `local_summary_cancelled`; legacy `local_meeting_summary_*` events may still appear during compatibility windows. Settings actions also cover retry/open/dismiss notice actions. | There is no direct feedback/usefulness event, and finished summaries are not yet part of the main product loop dashboard. | Existing: `provider`, `runtime`, `summary_action`, `setup_ready`, `queue_depth_bucket`, `chunk_count_bucket`, `duration_bucket`, `failure_kind`, `stage`. Add `result` or `feedback_bucket` only if a real user-visible feedback control exists. | Add a local-summary funnel: transcript saved -> summary requested -> finished/failed/cancelled -> opened/copied via settings actions. | `TranscriptedSettingsView.swift` summary request/finish/fail/cancel/action call sites. | `AnalyticsEventPolicyTests` pins local summary events through taxonomy parity; sanitizer tests cover forbidden payloads. | Small dashboard PR first. Product-code PR only when there is a real feedback action to instrument. |
