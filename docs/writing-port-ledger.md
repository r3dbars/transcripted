...
| `Sources/TildeCore/Engine/GhostBrainWire.swift` | `Sources/TranscriptedWriting/Core/Engine/GhostBrainWire.swift` | ported | 1 | Phase 3 deviation: an `unsupported` outcome with `rejectedEventIDs`, and the probe that finds history events of an unknown version (see History retries below). |
...
| `Sources/TildeCore/PersonalHistory/PersonalHistoryEvent.swift` | `Sources/TranscriptedWriting/Core/PersonalHistory/PersonalHistoryEvent.swift` | ported | 1 | Phase 3 deviation (plan decision 10): a text-free `.deletion` source that travels as version 2 with `deletedCharacters` (UTF-16 units, see Backspace counts below); typed and accepted events stay version 1, byte for byte. The capture policy takes the Writing app scope (`.outsideAppScope`). Helpers live in `PersonalHistoryDeletion.swift` and `WritingAppScope.swift`. |
...
| `Sources/TildeCore/PersonalHistory/PersonalVocabularyShadow.swift` | `Sources/TranscriptedWriting/Core/PersonalHistory/PersonalVocabularyShadow.swift` | ported | 1 | Phase 3 deviation: `consume` skips `.deletion` events, so the predictor learns exactly what Tilde's did. |
...
| `Sources/InlineGhostIME/GhostStats.swift` | `Sources/TranscriptedKeyboard/GhostStats.swift` | ported | 1 | Queue label renamed. The new `unsupported` outcome counts as an invalid request (completions never get it). |
...
- **History retries (phase 3 review):** Tilde retried a history batch the app refused every 5 s forever, so one event an older app can't read (a version-2 deletion) blocked all text behind it. Now the app answers a version-1 history batch that holds events of an unknown version with a terminal `unsupported` line naming those events' IDs (`GhostBrainRequest.unsupportedPersonalHistoryEventIDs`), and the keyboard drops only them and resends the rest. Any event the app refuses 12 times (`invalid_request`, `error`, an `unsupported` line naming nothing it sent) is dropped too. Not reaching the app (`unavailable`, `timeout`) still retries as in Tilde, and version-1 traffic is unchanged.
- **Backspace counts (phase 3 review):** a deletion event's `deletedCharacters` counts UTF-16 units, not `Character`s. The keyboard reports the `utf16Length` of the character it inserted, and `WritingEntryComposer` removes that many units from the joined text. Counting `Character`s removed too much when a combining mark or joiner typed on its own merged into the character before it. Plain Backspace also passes the real Secure Event Input state to the capture permit (it was always `false`, safe only because of an earlier return).
