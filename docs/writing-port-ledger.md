...
| `Sources/TildeCore/PersonalHistory/PersonalHistoryEvent.swift` | `Sources/TranscriptedWriting/Core/PersonalHistory/PersonalHistoryEvent.swift` | ported | 1 | Phase 3 deviation (plan decision 10): a text-free `.deletion` source that travels as version 2 with `deletedCharacters`; typed and accepted events stay version 1, byte for byte. The capture policy takes the Writing app scope (`.outsideAppScope`). Helpers live in `PersonalHistoryDeletion.swift` and `WritingAppScope.swift`. |
