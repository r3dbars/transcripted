...
2. **Split `MeetingSessionController` along its seams.** `[effort: L]` `[risk: med]`
   Keep capture lifecycle and the app-level transcription queue in focused
   coordinators. Leaves a smaller session state machine and reduces
