...
| `mic_only` | `true` | Flat; the user picked "Record Just My Mic", so only their side was recorded on purpose and the other side of any call is not in the transcript. Not a capture problem: the grade is unaffected and `system_audio_missing` is never set with it. Omitted for two-sided meetings. |
