...
- Status: running
- Current best: fresh-window scorer with per-stage stop breakdown plus fresh dictation and meeting baselines
...
- Planned knobs: stop timing instrumentation kept; fresh stop baseline captured; stop stage scorer kept; model eager warmup tested/rejected for default-on; model poll interval tested/rejected; file prefetch delay ruled out for this cached-model speed pass; auto-enter delay not run; paste/save ordering ruled out as first knob; fresh-window scorer kept; dictation fresh RTF passing; meeting fresh RTF passing with one sample
- Last attempt: attempt 7, model poll interval 100ms was rejected because first-start changed only 5-6ms
