...
- `dictating.svg`: the same bubble filled in, with the T's stem and the side bars cut out
...
T's stem and one pair of side bars, halfway between the stem and the wall. The mark sits in
a 700-unit square (about 16 pt tall in the 18 pt image) so it has the same breathing room as a
menu bar SF Symbol.

`Sources/UI/MenuBar/MenuBarGlyph.swift` draws the same shapes in code. Keep the numbers in
`make_menu_bar_icons.py` and that file in sync; `Tests/StatusItemPresentationTests.swift` fails
when they drift.

Regenerate: `python3 make_menu_bar_icons.py` (from anywhere), then, from this directory,
