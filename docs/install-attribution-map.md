...
| In-app update download | `update_check_finished`, `update_download_started`, `update_download_finished`, `update_ready_to_install`, `update_relaunching`, `update_installed` | PostHog | App-side update funnel. Use version-scoped aggregate counts. `update_installed` fires on the first launch of any newer version; filter `install_kind` in (`restart`, `quit`) for the in-app funnel, `unattributed` is a DMG, Homebrew or older-build install. |
...
| First useful artifact | `activation_first_artifact_saved` | PostHog | Strict first saved dictation, live meeting, or imported meeting Markdown artifact, emitted once per install without inspecting content. |
