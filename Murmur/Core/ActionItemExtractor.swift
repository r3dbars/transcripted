...
enum ActionItemExtractor {
    ...
    /// Identify speakers with graceful fallback on error
    static func identifySpeakersWithFallback(
        from transcript: String,
        speakerIds: [String],
        userName: String,
        apiKey: String
        ...
        do {
            return try await identifySpeakers(from: transcript, speakerIds: speakerIds, userName: userName, apiKey: apiKey)
        } catch {
            print("⚠️ Speaker identification failed, using generic labels: \(error)")
            return SpeakerIdentificationResult(speakers: [], userSpeakerId: nil)
        }
    }

    // MARK: - Build Speaker Mappings

    /// Build speaker mappings from Gemini identification results
    /// - Parameters:
    ///   - geminiResult: Result from speaker identification
    ///   - allSpeakerIds: All speaker IDs detected by AssemblyAI
    ///   - userName: The configured user name
    /// - Returns: Dictionary mapping speaker IDs to SpeakerMapping
    static func buildSpeakerMappings(
        from geminiResult: SpeakerIdentificationResult,
        allSpeakerIds: Set<String>,
        userName: String
    ) -> [String: SpeakerMapping] {
        var mappings: [String: SpeakerMapping] = [:]

        // Initialize all speaker IDs with generic names
        for id in allSpeakerIds {
            mappings[id] = SpeakerMapping(speakerId: id, identifiedName: nil, confidence: nil)
        }

        // Apply Gemini identifications
        for speaker in geminiResult.speakers {
            if let speakerId = speaker.speakerId, mappings[speakerId] != nil {
                mappings[speakerId] = SpeakerMapping(
                    speakerId: speakerId,
        ...
        // Mark user's speaker ID if identified
        if let userSpeakerId = geminiResult.userSpeakerId, mappings[userSpeakerId] != nil {
            mappings[userSpeakerId] = SpeakerMapping(
                speakerId: userSpeakerId,
                identifiedName: userName,
                confidence: "high"
            )
        }
    ...
    /// Generate a preliminary transcript text from AssemblyAI result for Gemini processing
    /// This creates a simple text format that Gemini can parse for speaker identification
    static func generatePreliminaryTranscript(from result: CombinedAssemblyAIResult) -> String {
        var lines: [String] = []

        // Collect all utterances from both sources
        var allUtterances: [(timestamp: Int, speaker: String, text: String, source: String)] = []

        if let mic = result.micResult {
            for utterance in mic.utterances {
                allUtterances.append((utterance.start, utterance.speaker, utterance.text, "Mic"))
            }
        }

        if let sys = result.systemResult {
            for utterance in sys.utterances {
                allUtterances.append((utterance.start, utterance.speaker, utterance.text, "System"))
            }
        }

        // Sort by timestamp
        allUtterances.sort { $0.timestamp < $1.timestamp }

        // Format as simple transcript
        for utterance in allUtterances {
            let minutes = utterance.timestamp / 60000
            let seconds = (utterance.timestamp / 1000) % 60
            let timestampStr = String(format: "[%02d:%02d]", minutes, seconds)
            lines.append("\(timestampStr) [Speaker \(utterance.speaker)] \(utterance.text)")
        }
