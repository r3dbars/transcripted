...
- `bash scripts/dev/linux-checks.sh` — runs every check that works on Linux without Swift (agent contract self-tests, syntax, build-source lists, duplicate declarations, analytics/telemetry/privacy gates, the strict release-health fixture gate for the current `Info.plist` version, every `scripts/ops` + `scripts/release` `--self-test`, script test suites, Swift source-pin mirror). Prints `PASS`/`FAIL`/`SKIP` per check with elapsed time and the exact command to re-run it; exits non-zero on any failure. Writes only under `build/linux-checks/`. Flags: `--quick`, `--only <substring>`, `--list`, `--verbose`, `--strict-tools` (what repo-hygiene CI runs: missing ruby/`origin/main` fails instead of skipping)
