...
#   lab     64 meetings  ~3.5 GB   same, roughly double
...
case "$ARG" in
  ...
  # Speaker lab cross-call set: the 8 Edinburgh series above plus Idiap (IS) and TNO (TS)
  # series (16 groups x 4 sessions, all with pyannote RTTMs), so returning-speaker recognition
  # is measured across three rooms and mic setups, not just one site.
  lab)     MEETINGS="$(expand_series ES2002 ES2003 ES2005 ES2006 ES2007 ES2008 ES2009 ES2010 \
              IS1000 IS1001 IS1003 IS1004 TS3003 TS3004 TS3005 TS3006)" ;;
