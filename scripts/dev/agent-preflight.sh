...
if [ -n "$changed_paths" ]; then
    while IFS= read -r path; do
        ...
        if matches_any "$path" "scripts/ops/nightly-security-check.py"; then
            ...
            add_command "python3 scripts/ops/nightly-security-check.py --strict --automation-toml Tests/Fixtures/nightly-security-automation.toml --write-report build/nightly-security-report.json"
        fi
