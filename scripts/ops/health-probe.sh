...
probe_posthog() {
  ...
  local workflow_events onboarding_events first_value_events speaker_review_events query daily_query payload daily_payload
  workflow_events="'app_launched','app_unclean_shutdown_detected','app_session_stall_detected','onboarding_completed','dictation_started','dictation_start_failed','dictation_completed','dictation_cancelled','dictation_no_speech','dictation_audio_route_recovery_timeout','meeting_recording_started','meeting_recording_start_failed','meeting_recording_stopped','meeting_recording_cancelled','meeting_file_imported','meeting_transcript_saved','meeting_transcript_failed','meeting_transcript_skipped','meeting_speaker_review_prompted','meeting_speaker_review_actioned','meeting_speaker_review_completed','activation_first_artifact_saved','activation_artifact_action_clicked','activation_agent_prompt_action_clicked','activation_agent_setup_cta_clicked','activation_return_proxy_observed'"
  ...
  speaker_review_events="'meeting_speaker_review_prompted','meeting_speaker_review_actioned','meeting_speaker_review_completed'"
  query="select uniq(distinct_id) as devices_7d, sum(case when event in ($workflow_events) then 1 else 0 end) as workflow_events_7d, sum(case when event in ($onboarding_events) then 1 else 0 end) as onboarding_events_7d, sum(case when event in ($first_value_events) then 1 else 0 end) as first_value_events_7d, sum(case when event in ($speaker_review_events) then 1 else 0 end) as speaker_review_events_7d from events where timestamp >= now() - interval 7 day"
  ...
  local devices events onboarding first_value speaker_review daily_devices
  ...
  speaker_review=$(echo "$response" | jq -r '(.data // .results)[0][4] // 0')
  daily_devices=$(echo "$daily_response" | jq -r '((.data // .results // []) | map("\(.[0])=\(.[1])") | join(", "))')
  if [[ -z "$daily_devices" ]]; then
    daily_devices="none"
  fi
  echo "PostHog (last 7d): devices=$devices, workflow_events=$events, onboarding_events=$onboarding, first_value_events=$first_value, speaker_review_events=$speaker_review"
