...
probe_posthog() {
  ...
  workflow_events="'app_launched','app_unclean_shutdown_detected','app_session_stall_detected','onboarding_completed','dictation_started','dictation_start_failed','dictation_completed','dictation_artifact_saved','dictation_cancelled','dictation_no_speech','dictation_audio_route_recovery_timeout','meeting_recording_started','meeting_recording_start_failed','meeting_recording_stopped','meeting_recording_cancelled','meeting_file_imported','meeting_transcript_saved','meeting_transcript_failed','meeting_transcript_skipped','activation_first_artifact_saved','activation_second_artifact_saved','activation_artifact_action_clicked','activation_agent_prompt_action_clicked','activation_agent_setup_cta_clicked','agent_capture_query_observed','activation_return_proxy_observed','onboarding_exited','workflow_abandoned'"
  onboarding_events="'onboarding_shown','onboarding_step_viewed','onboarding_permission_cta_clicked','onboarding_permission_status_changed','onboarding_model_state_changed','onboarding_primary_cta_clicked','onboarding_first_dictation_started','onboarding_first_dictation_saved','onboarding_first_dictation_stop_clicked','onboarding_first_dictation_empty','onboarding_meeting_dry_run_clicked','onboarding_agent_cta_clicked','onboarding_reporting_toggle_changed','onboarding_completed','onboarding_dismissed','onboarding_exited'"
  first_value_events="'dictation_artifact_saved','dictation_completed','onboarding_first_dictation_saved','meeting_transcript_saved','onboarding_agent_cta_clicked','activation_first_artifact_saved','activation_second_artifact_saved','activation_artifact_action_clicked','activation_agent_prompt_action_clicked','activation_agent_setup_cta_clicked','agent_capture_query_observed','activation_return_proxy_observed'"
  query="select uniq(distinct_id) as devices_7d, sum(case when event in ($workflow_events) then 1 else 0 end) as workflow_events_7d, sum(case when event in ($onboarding_events) then 1 else 0 end) as onboarding_events_7d, sum(case when event in ('workflow_abandoned','onboarding_exited') then 1 else 0 end) as abandonment_events_7d, sum(case when event in ($first_value_events) then 1 else 0 end) as first_value_events_7d from events where timestamp >= now() - interval 7 day"
  ...
  local devices events onboarding abandonment first_value daily_devices
  ...
  abandonment=$(echo "$response" | jq -r '(.data // .results)[0][3] // 0')
  first_value=$(echo "$response" | jq -r '(.data // .results)[0][4] // 0')
