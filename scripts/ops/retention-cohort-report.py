...
def saved_artifact_predicate(alias: str = "") -> str:
    ...
    return (
        ...
        f"OR ({prefix}event = 'activation_second_artifact_saved' "
        f"AND {prefix}properties['second_artifact_kind'] = 'dictation') "
...
def run_self_test() -> None:
    ...
    assert "second_artifact_kind'] = 'dictation'" in queries["summary"]
    for query_name in ("summary", "first_artifact", "first_run"):
        assert "activation_second_artifact_saved" in queries[query_name]
    for name, query in queries.items():
