...
run_route_automation_proxy_suite() {
  ...
  record_route_automation_proxy \
    ...
    "Bluetooth headset fallback policy, mocked connect/disconnect, output-only Bluetooth, built_in_input_to_bluetooth_output route shape, sample-rate settling, route readiness, preferredBuiltInForBluetoothHeadset, builtInFallbackSuppressedForRecoveryAttempt, routeNotSettled, audio_route_not_settled, hfp_suspected, stale-route readiness, zombie-recovery state transitions" \
    "bash run-tests.sh" \
