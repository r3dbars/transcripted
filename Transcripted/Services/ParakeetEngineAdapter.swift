...
//
// Step 10 of the TranscriptedCore extraction (merge-plan §5.1): Core is STT-agnostic,
// so the Parakeet implementation lives in the Transcripted app target and conforms to
// TranscriptedCore's `SpeechToTextEngine` protocol. Draft supplies its own adapter in
// `Sources/Meeting/` (Lane B). This file replaces the pre-extraction `ParakeetService.swift`
// deleted in commit ba6685d.
...
@MainActor
final class ParakeetEngineAdapter: ObservableObject, SpeechToTextEngine {
    ...
    nonisolated func transcribeSegment(samples: [Float], source: AudioSource) async throws -> String {
        guard let manager = await MainActor.run(body: { self.asrManager }),
              await manager.isAvailable else {
            throw PipelineError.modelNotLoaded(model: "Parakeet")
        }
