...
struct SavedPillView: View {
    ...
    private func animateEntrance() {
        ...
        Task { @MainActor in
            // Phase 4: Fade in content (overlaps with phase 1)
            try? await Task.sleep(for: .seconds(0.15))
            withAnimation(.easeOut(duration: 0.2)) { contentOpacity = 1.0 }

            // Phase 2: Slight bounce
            try? await Task.sleep(for: .seconds(0.10))
            withAnimation(.spring(response: 0.2, dampingFraction: 0.5)) { checkScale = 1.1 }

            // Phase 3: Settle
            try? await Task.sleep(for: .seconds(0.15))
            withAnimation(.spring(response: 0.15, dampingFraction: 0.8)) { checkScale = 1.0 }
        }
    }
