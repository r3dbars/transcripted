...
/// Markdown (parsed by `CaptureSummaryParser`); items without a marker are
/// open. The tool handler still rejects `done` with an explicit error until
/// the marker write-back path is verified against a real library — the index
/// filter underneath (`TranscriptIndex.listActionItems`) already supports it.
