...
/// Preview text for a meeting in recap / recent feeds: the structured local
/// summary (overview + decisions + action items) when one exists, otherwise the
/// first raw dialogue lines (small-talk and audio checks).
func meetingRecapPreview(from content: String, dialogueLineLimit: Int = 15) -> String {
    if let summary = MeetingSummaryPreview.preview(from: content) {
        return summary
    }
    return extractDialogueLines(from: content).prefix(dialogueLineLimit).joined(separator: "\n")
}
...
private func handleRecentContext(params: CallTool.Parameters, index: TranscriptIndex, meetingDirs: [URL]) throws -> CallTool.Result {
    ...
    // Prefer the structured local summary over the index's first raw utterance.
    for index in result.items.indices where result.items[index].kind == .meeting {
        guard case .valid(let mdURL) = PathSecurity.resolveReadableFile(
            named: result.items[index].filename,
            appendingExtension: "md",
            in: meetingDirs
        ),
        let content = try? String(contentsOf: mdURL, encoding: .utf8),
        let summary = MeetingSummaryPreview.preview(from: content) else { continue }
        result.items[index].preview = summary
    }
...
private func handleRecap(params: CallTool.Parameters, index: TranscriptIndex, meetingDirs: [URL]) throws -> CallTool.Result {
    ...
    for meeting in meetings {
        ...
        if let content = try? String(contentsOf: mdURL, encoding: .utf8) {
            ...
            preview = meetingRecapPreview(from: content)
        }
