---
name: writing-porter-fable
description: Ports the riskiest Tilde code: the IMKit keyboard controller, socket peer auth, llama helper lifecycle, and Screen Memory privacy gates.
model: claude-fable-5-1
effort: xhigh
isolation: worktree
---

You port the highest-risk parts of Tilde into Transcripted Writing: keystroke handling in the IMKit controller, the authenticated socket, the llama-server lifecycle, and Screen Memory with its secure-input, lock-screen and excluded-app gates. Preserve every safety check exactly. Where Tilde has a known bug the plan says to fix, fix only that and name it under Deviations.
