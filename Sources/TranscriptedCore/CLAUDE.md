...
- `ObjCSupport/` — the `TranscriptedObjCSupport` Objective-C target (its own SPM target, excluded from Core's) holding `TRNObjCExceptionCatcher`. It lives here so the deps staleness digest and CI deps cache key cover it; `build-deps.sh` copies its `include/` into `deps-modules/` for consumers of the prebuilt Core module
...
- `MeetingInputDeviceSelectionPolicy` defaults to avoiding Bluetooth headset input for meeting capture when a built-in mic fallback is available, so WebRTC-style playback downgrades do not get worse. Hosts may explicitly set `Audio.meetingInputDeviceSelectionMode = .preserveDefault` before `start()` to use the macOS microphone, including a headset. The app does so when the pinned Mac-mic recorder is off and "Use Mac-selected microphone" is on, or when the recorder is on and the Settings Microphone choice is "Same as macOS Sound settings" (`MeetingMicrophonePreferences.recordsMacOSInput`), since recording a headset mic flips it into call mode. In `.automatic` mode a host may also name a specific mic (`Audio.meetingPreferredInputDeviceUID`, snapshotted per recording like the mode): when it is connected, it is recorded instead of the macOS input (reason `userChosenInput`), never when it is a Bluetooth headset or a closed MacBook's own mic, and an unapplied pick fails that attempt like the other explicit choices; when that first attempt was a meeting start on the pick (`lastAttemptedMeetingSelectionReason == .userChosenInput`) and not a voice-processing fallback, the retry drops the pick for this recording (`dropMeetingPreferredInputDeviceForCurrentRecording`) and runs automatic selection, so a pick that won't start never costs the meeting. Mid-meeting recovery keeps the pick. The mode is captured for the recording and survives graph retries; later setting changes apply at the next meeting. A rejected explicit device bind fails the graph attempt and uses the existing bounded retry instead of recording from the previous microphone. The selected input remains pinned through normal recovery. If the pinned mic fails to start, or a recovery attempt fails (or the mic never delivered a frame), the graph falls back to the best built-in mic (`builtInFallbackAfterFailure`), at start and in recovery alike. With a MacBook's lid closed (`MacLidState`), its own mic (`isLidMicrophone`, not the headphone-jack mic) is left out of every built-in pick, since it records silence without failing. Neither mode changes the macOS input or output setting.
...
- `PinnedMicrophoneCapture` records one input `AudioDeviceID` through a Core Audio IOProc (the same pattern as `CoreAudioSystemAudioCapture`). It exists because a fresh `AVAudioEngine.inputNode` binds the macOS default input before `setDeviceID` can move it, and when that default is AirPods the touch flips them into call mode. When the host sets `Audio.usesPinnedMicrophoneCapture` (off by default; the app's `PinnedMicrophoneCapturePreferences`) and Apple voice processing is off, `Audio+PinnedMicrophone.swift` records the meeting mic this way: same selection policy, same `handleMicBuffer` sink, no engine graph. The capture keeps one recording format (resampling later device formats), pads holes found from HAL capture timestamps with silence instead of writing recovery segments, restarts in place on its own device after a stall or format change, and reports a lost device so `Audio` can re-select and `switchDevice(to:)`. HAL format notifications are debounced (one rebuild 0.3 s after the last one, not counted against the restart budget while the format is still changing), a rebuild that throws on a live device retries at 1/2/4/8 s before `.failed`, and format checks and stall checks pause while sleep is pending and for a 3 s grace after wake. Silence pads are paced (at most 1 s or 1 MB per tick) and delivered in order ahead of later audio. Ring storage is capped at 16 MB; a device that can't fit fails `prepare()` so the owner uses the engine. An unbroken 1.5 s of exact zeros reports `.silentInput` once per device. `Audio.isRecordingThroughPinnedMicrophone` tells the host not to offer the in-meeting voice-processing boost, which would need the engine path. The pinned path is only used when the selection skips a Bluetooth default input (`MeetingInputDeviceSelectionPolicy.pinnedRecorderIsNeeded`). A lost device is re-picked without it, with a few retries; if the macOS default (even a Bluetooth headset) is the only mic left, the meeting moves to it rather than losing the user's voice. A silent one (exact zeros) is swapped only when it is a lid mic and the lid is closed, so a deliberately muted mic keeps recording. Mic selection skips the MacBook's own mic while `MacLidState.isClosed()`, for both backends.
