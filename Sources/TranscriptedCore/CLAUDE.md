...
- `Audio` resolves its system-audio backend through `SystemAudioCaptureEngine`; `CoreAudioSystemAudioCapture` uses Core Audio process taps for System Audio Recording Only permission. Tests inject stubs through the same seam. The legacy `SCKAudioCapture` implementation remains for its regression tests, not as an automatic permission-expanding fallback.
