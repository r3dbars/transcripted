...
- `Audio` reaches system audio through the `SystemAudioCaptureEngine` seam; `SCKAudioCapture` (ScreenCaptureKit) is the only production backend, and tests inject stubs through the same protocol.
