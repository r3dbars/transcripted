...
- `DictationPersistentInputPreferences.swift` — retired faster-Bluetooth-dictation opt-in (no Settings toggle; `retireFasterBluetoothDictation()` switches it off at launch), preferred CoreAudio device UID, and crash-recovery ownership marker. Kept one release so `PersistentDictationInputController` can hand back the user's previous Mac-wide mic; delete both together after that
