...
- `CaptureLibraryPathSafety.swift` — the ONE canonical, dependency-free (pure Foundation) definition of the save-path safety predicate (system-directory rejection with a home-directory escape hatch). Vendored by real git symlink into `Sources/TranscriptedCore/Services/CaptureLibraryPathSafety.swift` and `Tools/TranscriptedCaptureKit/Sources/TranscriptedCaptureKit/CaptureLibraryPathSafety.swift` so `RecordingValidator.validateSavePath` and `CaptureLibraryResolver.validatedConfiguredDirectory` delegate to the same rule as `TranscriptedStoragePreferences.isSafeCaptureLibraryURL` here. Edit only this file — see its header comment for how each vendored copy (including `scripts/entrypoints/build-deps.sh`'s isolated `$DEPS_BUILD` copy) picks up the change.
