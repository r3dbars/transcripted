...
- `CaptureLibraryMigrationPlanner.swift` — planning and execution for capture-library relocation: detects captures in the old folder, enumerates meeting Markdown plus retained `audio/*_audio/` directories plus dictation day files plus Writing day files (`writing/*.md`), and skips destination collisions. Copy never deletes originals. Move is copy, switch, then `removeOriginals` sends each copied original to the Trash only if it is unchanged since its copy (fingerprinted by size, file count, and newest mtime) and its copy still exists; skipped collisions always stay. `CaptureLibraryMoveSummary` is the status line
...
- `DictationPersistentInputPreferences.swift` — persisted faster-Bluetooth-dictation opt-in (reads as off while the Mac mic recorder can handle dictation, i.e. it's on and Apple voice processing isn't chosen, so the controller hands back any input it switched; `isStoredOn` is the raw toggle), preferred CoreAudio device UID, and crash-recovery ownership marker
...
- `MeetingInviteeSuggestionPolicy.swift` — Foundation-pure rules for calendar-invite name suggestions in speaker review: which event counts (only one whose start lines up with the recording start, in the same window as the record-this-meeting pop-up, so a later call inside a long calendar slot gets no names), cleaning invitee names (no raw emails), invitees first in the name list, and the 1:1 pre-fill. Suggestions only; it never changes silent auto-naming. The EventKit side is `Sources/Meeting/MeetingInviteeCalendarReader.swift`
...
- `TranscriptedPermissionAccess` plus `TranscriptedPermissionKind` are the app-level permission seams. UI flows should call into them instead of duplicating TCC branching, metadata, or user-facing permission copy. Meetings only need the narrow `systemAudioRecording` tier and never request full Screen Recording. Full Screen Recording is only for Writing's autocomplete, if the user turns it on. Writing owns that grant, so it is deliberately not a `TranscriptedPermissionKind` case: one would add a Screen Recording row to onboarding and Settings for everyone.
...
- `Tests/MeetingInviteeSuggestionPolicyTests.swift`
