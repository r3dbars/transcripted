...
- `CalendarPreArmPreferences.swift` — persisted (default-on) opt-in / kill switch for the calendar pre-arm path; gates the calendar-candidate path in `MeetingPromptDetector` (on top of calendar permission) and backs the General "Pre-arm from calendar" toggle and onboarding's "Meeting reminders" (see `docs/meeting-capture-prearm-implementation.md`)
- `MeetingTitlePrivacyPreferences.swift` — persisted (default: show real titles) toggle deciding whether the floating meeting prompt shows the real calendar event title or a generic "Meeting" label; consumed by `MeetingArmedPromptCopyPolicy` and the General "Show real meeting titles" toggle
