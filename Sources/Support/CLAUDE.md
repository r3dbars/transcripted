...
- `MeetingInviteeSuggestionPolicy.swift` — Foundation-pure rules for calendar-invite name suggestions in speaker review: which overlapping event counts, cleaning invitee names (no raw emails), invitees first in the name list, and the 1:1 pre-fill. Suggestions only; it never changes silent auto-naming. The EventKit side is `Sources/Meeting/MeetingInviteeCalendarReader.swift`
