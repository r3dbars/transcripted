...
- `DictationPersistentInputPreferences.swift` — retired faster-Bluetooth-dictation opt-in (no Settings toggle; `retireFasterBluetoothDictation(pinnedRecorderEnabled:)` switches it off at launch where the pinned recorder is on), preferred CoreAudio device UID (still live: set by the Settings Microphone picker and honored by the pinned recorder through `preferredDeviceUID()`, so the retire step keeps it), and crash-recovery ownership marker. The toggle key and marker stay one release so `PersistentDictationInputController` can hand back the user's previous Mac-wide mic; delete the controller and those then, but keep the preferred-UID accessors
