...
- `scripts/ops/dictation-stop-autoeval.sh --encoder-compute cpu-and-gpu` is the local-only encoder compute comparison. Production keeps FluidAudio's default unless that benchmark process sets `TRANSCRIPTED_PARAKEET_ENCODER_COMPUTE_UNITS=cpu_and_gpu`.
