...
- `ParakeetZombieEngineRecovery.swift` — `ParakeetEngine` extension: the startup audio watchdog that detects zombie engines (running but no usable signal after sleep/wake) and the separate generation-gated recovery task that replaces the stale `AVAudioEngine` through a bounded reset and retries once; the audio-graph/queue owner-token helpers stay in `ParakeetEngine.swift` because every start/stop path uses them
