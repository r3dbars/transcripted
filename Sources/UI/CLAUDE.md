...
- `Overlay/MeetingLiveTranscriptDrawerView.swift` — self-contained drawer container (title, copy, open-in-browser, status line, scrolling transcript, drag-to-resize grip) shown below the recording strip, clipped and faded as one unit while the panel resizes
