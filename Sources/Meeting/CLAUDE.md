...
- `MeetingMicBoostPromptPolicy` is the canonical place for the in-meeting Boost Mic consent prompt. It must not present or apply actions after recording stop/cancel/termination teardown begins. A call app that was merely open at start doesn't block the prompt; `MeetingCaptureBridge.armVoiceProcessingForActiveRecording` refuses only while a call app holds the mic, waits out a mic recovery instead of dropping the boost, and, when it looked past an open call app, watches for that app taking the mic and hands it back. A boost that never applied is recorded as `shown`, not `accepted`, so the Home hint stays.
