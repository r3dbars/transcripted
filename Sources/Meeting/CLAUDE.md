...
- `MeetingArmedPromptCopy.swift` — Foundation-pure copy + one-shot chime policy for the gentle calendar "armed" prompt: the `Starts in N min` pre-arm subtext, the single `Starting now — tap to record` nudge, and the privacy-resolved title (real event title vs generic "Meeting"). Consumed by `MeetingOverlayController`'s calendar-prompt display; it only decides what the card says and never starts a recording (see `docs/meeting-capture-prearm-implementation.md`)
...
- Calendar pre-arm prompting is gated by `CalendarPreArmPreferences` (default on) on top of calendar permission: the detector only adds calendar candidates when `calendarAccessGranted() && isCalendarPreArmEnabled()`. Turning it off stops calendar prompts and the calendar title hint while leaving the runtime/mic ad-hoc fallback running — the feature degrades, it does not die. The load-bearing invariant holds regardless: detection only *requests* a prompt (`onPromptRequest`); a recording file is written only on the explicit Record tap (`MeetingOverlayController.onPromptRecord` → `MeetingSessionController.startRecording`), never by the detector.
