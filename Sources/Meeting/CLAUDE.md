...
- `BrowserCallEvidence.swift` — Foundation-pure "is this browser mic use really a call?" policy. Classifies browser window titles (a Meet/Teams/Zoom/Webex tab names the call; a focused ChatGPT/Claude/Loom/Google Docs window is not a call) and decides prompt-now / wait / never from the title, the camera, whether the same browser is playing audio, and how long it has held the mic (20 s corroborated, 60 s mic only). Titles never leave this decision
- `BrowserWindowTitleReader.swift` — reads browser window titles through the Accessibility permission the app already holds for paste-back (no new permission; returns nothing when untrusted), bounded by a short AX messaging timeout. Titles are classified in memory and dropped, never logged or sent
- `MeetingPromptLearnedBackoff.swift` — persisted per-kind "Not now" learning for ad-hoc prompts (`native:<provider>`, `browser_verified`, `browser_unverified`): consecutive Not nows stay quiet longer (30m, 2h, 8h, and a day for unrecognized browser mic), a recording resets it, and an unrecognized browser mic turned down three times with no recording ever stops prompting until the streak is forgotten (14 days)
...
- A browser holding the mic is not proof of a call (ChatGPT voice, web dictation, Loom and voice search all do it; in 30 days to 2026-09-23 the browser prompt was turned down 2.4 times per recording). `MeetingPromptDetector.adHocCandidate` runs every browser signal through `BrowserCallEvidence`: a window title naming a call surface prompts at once under its real provider (a Teams tab is `.teams`, id `mic:teams`; `Candidate.callEvidence == .tabTitle`); a focused known non-call site never prompts (`not_a_call` suppression, and the session feeds neither the funnel event nor the missed-call nudge); otherwise the prompt waits for continuous mic use (20 s when the same browser is also playing audio or the camera is on, 60 s otherwise) and uses the generic id `mic:browser`. The camera only corroborates: camera-on browser prompts were recorded less often than camera-off ones. Keep the generic and named ids apart so a Not now to one does not silence the other, and keep the unverified dismissal off the provider-wide `runtimeSuppressedUntil`.
- Explicit Not now on an ad-hoc prompt also feeds `MeetingPromptLearnedBackoff` (persisted only when the detector is built with `learnedBackoffDefaults`, which `TranscriptedApp` passes as `.standard`; tests stay in memory) and marks that kind declined for the rest of the detected call. A meeting recording during a detected call (from the prompt, the menu or the hotkey) counts as a yes and clears the learned quiet.
