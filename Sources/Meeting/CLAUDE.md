...
- `MeetingImportedAudioPreparer.swift` — copies imported recordings into app-managed scratch paths and owns the privacy-minimal imported-job journal. Journal sessions hold a per-job process lease and retain the record through transcript commit until scratch cleanup or durable failed-queue handoff
...
11. Imported jobs use their journal UUID as the transcript UUID. Recovery skips a live lease owner, rejects non-regular scratch entries, and treats either the committed phase or a matching saved transcript as proof that only scratch cleanup remains.
