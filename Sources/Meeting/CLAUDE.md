...
- A live recording under 2 seconds, or one under `TranscriptionTaskManager.accidentalStartMaximumLength` (10 s) that ends in `noSpeechDetected`, is an accidental start: Core deletes its scratch audio and journal, keeps no failed row, and publishes `DisplayStatus.discardedAccidentalStart`. The controller settles it as `TerminalTranscriptionOutcome.discarded` (a cancel: no "Saved" flash, no error, `meeting_transcript_skipped` with `failure_kind=accidental_start`, friction `cancelled`). Imports and anything with an unknown length never take this path.
