...
- Retry availability for a saved failed row is two independent questions, and both must be answered before the affordance is withdrawn. `FailedTranscription.isRetryable` asks whether the *failure kind* leaves anything to recover — single-source failures do (the pipeline drops an unusable mic and transcribes what survived), and so does `noSpeechDetected` (a judgment about words, not audio; a later pass or another model can find them), while `recordingTooShort` does not. `FailedRecordingSignalProbe` then asks the factual question of whether the surviving files hold signal. Never suppress retry on an unproven verdict: `FailedMeetingUsableAudio.unknown` must stay optimistic, because wrongly hiding the action strands recoverable audio with no way to reach it, which is the bug this policy replaced.
- A live recording is an accidental start when its files are under 2 seconds from a session under `accidentalStartMaximumSessionForShortFiles` (4 s) with clean health, or when it ends in `noSpeechDetected` with files and the controller's session clock (`sessionLength`, Record to Stop) both under `TranscriptionTaskManager.accidentalStartMaximumLength` (10 s), clean health (no gaps, device switches, missing/unusable track or degraded capture) and no track with speech-like modulation. Core then deletes its scratch audio and journal, keeps no failed row, and publishes `DisplayStatus.discardedAccidentalStart`. The controller settles it as `TerminalTranscriptionOutcome.discarded` (a cancel: no "Saved" flash, no error, `meeting_transcript_skipped` with `failure_kind=accidental_start`, friction `cancelled`). Short files from a longer or unhealthy session are a broken capture and stay a visible failure (retained when over 2 s; sub-2 s scratch is deleted as on main, with "capture stopped early" copy instead of "Nothing broke"). Imports, retries and unknown lengths never take this path.
