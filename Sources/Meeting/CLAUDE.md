...
- A browser holding the mic is not proof of a call (ChatGPT voice, web dictation, Loom and voice search all do it; in 30 days to 2026-09-23 the browser prompt was turned down 2.4 times per recording). `MeetingPromptDetector.adHocCandidate` runs every browser signal through `BrowserCallEvidence`, with titles read off the main actor (the first read of a session holds the prompt until it lands). A call-only title prompts at once under its real provider (a Teams meeting tab is `.teams`, id `mic:teams`; `Candidate.callEvidence == .tabTitle`) and sticks for the session. A focused known non-call site also sticks: `not_a_call` for the rest of that browser session even after a tab switch, and without a prompt or recording the session feeds neither the funnel event nor the missed-call nudge. A focused call app waits 20 s as `mic:browser-call`; anything else waits for continuous mic use (20 s when the same browser is also playing audio or the camera is on, 60 s otherwise) as `mic:browser`. A browser that lets go of the mic keeps its session for 30 s (Safari mutes that way). The camera only corroborates: camera-on browser prompts were recorded less often than camera-off ones. Keep the generic, call-site and named ids apart so a Not now to one does not snooze the others, and keep every dismissal that could not name its call (`MeetingPromptCallEvidence.quietsProviderOnDismiss == false`) off the provider-wide `runtimeSuppressedUntil`. Title re-reads stop once a browser prompt was produced or held back by the learning.
