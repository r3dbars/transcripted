...
- `MicActivityMonitor.swift` — Core Audio process-object watcher for ad-hoc call detection. Emits the set of non-self bundle IDs currently holding the mic input so the detector can prompt when a call *starts* (including a spontaneous Google Meet with no calendar invite), plus a second set of native conferencing bundle IDs confirmed to be playing audio *output* (`kAudioProcessPropertyIsRunningOutput`) — the listen-only / hard-muted call signal, scoped to conferencing families only so browser/media playback never becomes a call signal on its own and gated by a longer output sustain so notification dings stay quiet. A third set (`onBrowserOutputChange`) carries browser processes playing audio while that same browser holds the mic; it only shortens the browser wait, never prompts. Metadata-only, no TCC permission; CoreAudio confined to one serial queue. Listens on the default input device's running-somewhere edge but, because that edge silently misses calls on a non-default device or when the device is already running, leans on a short process-object backstop poll (seconds, not the original 60s) so a missed edge still surfaces fast. See `docs/auto-call-detection-spec.md`
- `BrowserCallEvidence.swift` — Foundation-pure "is this browser mic use really a call?" policy. Classifies browser window titles: a title that only exists during a call (a Meet tab, the Zoom web client, a Teams meeting window) names it from any window; a focused known non-call site (ChatGPT, Claude, Loom) is not a call; a focused call app whose title does not prove a call (Teams chat, a Slack huddle, WhatsApp) only corroborates; mail and calendar titles are skipped. Then decides prompt-now / wait / not a call from that, the camera, whether the same browser is playing audio, and how long it has held the mic (20 s corroborated, 60 s mic only). Titles never leave this decision
- `BrowserWindowTitleReader.swift` — reads browser window titles through the Accessibility permission the app holds for paste-back (no new permission; returns nothing when untrusted). Runs the AX round trips on a background queue under a 0.3 s overall budget, so a slow browser never stalls the main actor. Titles are classified in memory and dropped, never logged or sent
- `MeetingPromptLearnedBackoff.swift` — persisted per-kind "Not now" learning for ad-hoc prompts (`native:<provider>`, `browser_verified`, `browser_call_site`, `browser_camera`, `browser_unverified`): consecutive Not nows stay quiet longer (30m, 2h, 8h, and a day for unrecognized browser mic), a recording resets it, and an unrecognized browser mic turned down three times with no recording in 30 days stops prompting until the streak is forgotten (14 days). The detector only applies that learned-off rule when window titles were readable, and turning auto call detection off and on clears everything learned
...
- `MeetingSystemAudioAccessAlert.swift` — the two-button "can't hear the other side of the call" alert shown before a meeting starts when macOS says System Audio Recording is off; the decision logic (`MeetingSystemAudioAccessFlow`) lives in `MeetingRecordingStartGate.swift`
...
6. `MeetingSessionController.startRecording(...)` first runs `MeetingRecordingStartGate` so a missing microphone permission blocks before capture starts. System Audio Recording is read from macOS itself (`TranscriptedPermissionAccess.refreshSystemAudioRecordingStatusFromSystem`); when it is off, the user is asked first: "Turn It On" shows the macOS allow box (Don't Allow there records mic only, with no second question); after an earlier denial the button reads "Open System Settings", since macOS won't ask twice and "Record Just My Mic" starts a mic-only recording with no unverified-audio banner and no "unverified" flag on the saved meeting. After a denial that choice is remembered (`MeetingMicOnlyChoicePreference`) so later meetings skip the question until macOS's answer changes. Any "Record Just My Mic" start (`MeetingRecordingStartDecision.capturesSystemAudio == false`) records the mic alone: the bridge sets `Audio.capturesSystemAudio = false`, Core never builds the system-audio tap (so macOS isn't asked right after the user declined), and the start waits only on the mic. At stop, `MicOnlySilentSystemTrack` writes a sparse (no disk space on APFS) silent system WAV at the mic's sample rate and as long as the mic file, standing in for the silent track the old always-on tap left, so speaker review, Home re-transcribe and failed-meeting retries still see a two-track meeting (the rate must match: Home's playback mix takes its rate from the system track). The stop marks the health `systemAudioSkippedByChoice`, so the saved Markdown carries `mic_only: true` (the app's "Mic only" wording; a retry or Home re-transcribe keeps it without claiming a capture grade), `sources` lists only the mic, and telemetry reports capture_outcome `mic_only_by_choice`. Every failed-queue path (timed-out, quit, unexpected and queued stops, crash recovery via the journal) keeps the choice as `FailedTranscription.micOnlyByChoice`; a retry of such a row writes the silent track if it has none, then saves with the marker instead of `system_audio_missing`. "Turn It On" with no answer from macOS keeps the tap, since it can still raise the macOS box, and uses the permission-dialog timeout. It then uses `MeetingCaptureBridge` to start core audio capture into app-owned scratch paths. The bridge applies `MeetingMicrophonePreferences` before each start: automatic Bluetooth isolation by default, or macOS-selected input on opt-in. Core snapshots it for that recording and preserves its bounded route recovery.
...
- Ad-hoc call detection (`MicActivityMonitor` → `MeetingPromptDetector.updateMicInputUsers`) must never prompt while Transcripted itself holds the mic: it is gated by `isOwnCaptureActive` (meeting recording or dictation) and the monitor drops our own bundle ID by prefix. Browser calls map to `.googleMeet` via family-prefix matching because the mic is held by helper/service processes (`com.google.Chrome.helper`, `com.apple.WebKit.GPU`), and reuse the `.runtimeApp` source. The feature is behind `AutoCallDetectionPreferences` (default on).
- A browser holding the mic is not proof of a call (ChatGPT voice, web dictation, Loom and voice search all do it; in 30 days to 2026-09-23 the browser prompt was turned down 2.4 times per recording). `MeetingPromptDetector.adHocCandidate` runs every browser signal through `BrowserCallEvidence`, with titles read off the main actor (the first read of a session holds the prompt until it lands). A call-only title prompts at once under its real provider (a Teams meeting tab is `.teams`, id `mic:teams`; `Candidate.callEvidence == .tabTitle`) and sticks for the session. A focused known non-call site holds the prompt while in front, and when it was in front within 10 s of the browser taking the mic it sticks: `not_a_call` for the rest of that browser session even after a tab switch, and without a prompt or recording the session feeds neither the funnel event nor the missed-call nudge. Call apps are checked before non-call words, markers match whole words, and mail/calendar titles only ever count for a leading Meet code. A focused call app waits 20 s as `mic:browser-call`; anything else waits for continuous mic use (20 s when the same browser is also playing audio or the camera is on, 60 s otherwise) as `mic:browser`. A browser that lets go of the mic keeps its session for 30 s (Safari mutes that way). The camera only corroborates: camera-on browser prompts were recorded less often than camera-off ones. Keep the generic, call-site and named ids apart so a Not now to one does not snooze the others, and keep every dismissal that could not name its call (`MeetingPromptCallEvidence.quietsProviderOnDismiss == false`) off the provider-wide `runtimeSuppressedUntil`. The re-check timer stops once a browser prompt was produced or held back by the learning, and `evaluate()` presents nothing while a browser session's first title read is still running. Browser families are compared as app families (`com.apple.WebKit` is Safari).
- Explicit Not now on an ad-hoc prompt also feeds `MeetingPromptLearnedBackoff` (persisted only when the detector is built with `learnedBackoffDefaults`, which `TranscriptedApp` passes as `.standard`; tests stay in memory) and marks that kind declined for the rest of the detected call, capped at 8 hours. A Not now to any browser prompt declines every browser kind for that call, so focusing the Meet tab afterwards does not re-ask it by name. A meeting recording during a detected call (from the prompt, the menu or the hotkey) counts as a yes only for the kinds that call actually raised or held back; a recording started before any prompt was considered teaches nothing. A prompt held back by the learning also skips the missed-call nudge.
...
- The audio-output signal (`MicActivityMonitor.onOutputChange` → `MeetingPromptDetector.updateAudioOutputUsers`) is the third ad-hoc sensor, behind the same toggle and own-capture gates. It exists for the call the mic and camera both miss: a listen-only or hard-muted join where remote people are talking but nothing on this Mac holds the mic. Attribution is native conferencing families only (`MeetingPromptProvider.audioOutputProvider`) — browser output is never a call signal on its own (it only corroborates a browser that already holds the mic) — and the signal tiers below the mic and above the camera in `callSignals` (mic wins outright; output beats the camera because it has real process attribution; `.audioOutput` reason, same `mic:<provider>` candidate id).
...
- If a live stop finalizes microphone audio but no system-audio file, continue through the existing mic-only pipeline. It labels `system_audio_missing`, retains available mic audio beside the transcript, and must not turn the whole artifact into a failed-queue item. The exception is a "Record Just My Mic" meeting whose silent stand-in track couldn't be written: its health is marked `systemAudioSkippedByChoice`, so it keeps its grade (no `system_audio_missing`, not degraded) and saves `mic_only: true`.
...
- `Tests/BrowserCallEvidenceTests.swift`
- `Tests/MeetingPromptLearnedBackoffTests.swift`
