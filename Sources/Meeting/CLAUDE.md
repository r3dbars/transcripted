...
- `MicActivityMonitor.swift` — Core Audio process-object watcher for ad-hoc call detection. Emits the set of non-self bundle IDs currently holding the mic input so the detector can prompt when a call *starts* (including a spontaneous Google Meet with no calendar invite), plus a second set of native conferencing bundle IDs confirmed to be playing audio *output* (`kAudioProcessPropertyIsRunningOutput`) — the listen-only / hard-muted call signal, scoped to conferencing families only so browser/media playback never counts and gated by a longer output sustain so notification dings stay quiet. Metadata-only, no TCC permission; the watcher's CoreAudio reads are confined to one serial queue, and the static one-shot `currentMicInputBundleIDs` (used by Boost Mic) runs on a detached task. Listens on the default input device's running-somewhere edge but, because that edge silently misses calls on a non-default device or when the device is already running, leans on a short process-object backstop poll (seconds, not the original 60s) so a missed edge still surfaces fast. See `docs/auto-call-detection-spec.md`
...
- `MeetingMicOnlyNotice.swift` — the quiet "Mic only" note on the recording pill for a meeting that never built the system-audio tap (remembered mic-only after Don't Allow included), what clicking it does (macOS box if never asked, System Settings otherwise), the access re-check that flips it to "Call audio on", the detected-call prompt's call-audio-off line, and which mid-meeting system-audio warnings get Check Access
...
- `MeetingMicBoostPromptPolicy` is the canonical place for the in-meeting Boost Mic consent prompt. It must not present or apply actions after recording stop/cancel/termination teardown begins. A call app that was merely open at start doesn't block the prompt; one launched during the meeting does. `MeetingCaptureBridge.armVoiceProcessingForActiveRecording` refuses while a call app holds the mic or one launched during the meeting, waits out a mic recovery instead of dropping the boost, and, when it looked past an open call app, watches for that app taking the mic and hands it back. A boost that never applied, including one still retrying when Stop was pressed, is recorded as `shown`, not `accepted`, so the Home hint stays. A "can't hear the call" report still open at Stop only marks the saved meeting degraded when the loss was confirmed, an earlier cause already degraded it, or it lasted `unresolvedUnheardDegradeSeconds`; the likeliest open report is a call that ended before Stop.
...
- Live system-audio degradation is recording-scoped: a hard ScreenCaptureKit interruption may use its one existing bounded restart, while prolonged silence only raises a warning because silence can be legitimate. Keep the warning latched in the overlay through transient unknown status until the meeting ends, even after recovery or acknowledgement. Only an interruption or failure warning marks the saved capture health as degraded; a silence warning must not (it stamped ~77% of production meetings `degraded` and fired a degraded-capture diagnostic for each — every meeting where the local user did the talking, or Stop was pressed after the call wound down). Silence stays visible through `system_status` in the pipeline diagnostics snapshot. The exception is `unheardPlayback`: Core reports the tap heard only digital silence for a sustained stretch, after its own reconnects, while another app kept playing. That is probably a lost call, so it shows its own prompt and degrades the saved capture while it lasts. If the same tap then hears signal, the call was only quiet: the prompt goes away and nothing is degraded (unless an earlier interruption already was). Only signal that needed a new tap or output (`systemAudioDidLosePlayback`) moves it to "Call audio is back" and keeps the meeting degraded. Every recovered system-audio notice ("reconnected", "restored", "call audio is back") shows only an OK button and hides itself after `MeetingSystemAudioPromptPolicy.recoveredAutoHideSeconds` through the normal acknowledgement, so the saved meeting keeps its degraded mark; prompt copy says call audio "may be missing" rather than "marked degraded". The "System audio not verified" notice only shows when macOS does not confirm System Audio Recording is on; with access confirmed, a silent tap is a quiet Mac (usually an in-person meeting) and the unheard-playback warning covers the real loss. The audio-inactivity warning owns prompt precedence because it can auto-stop; keep system degradation icon-visible while inactivity is active, then present its prompt after inactivity clears when still applicable.
...
- `Tests/MeetingMicOnlyNoticeTests.swift`
