...
enum ImportedAudioQueuePersistenceFailureCopy {
    static let retryEntryMessage =
        "Imported audio was saved after Transcripted couldn't add it to the transcription queue. Finish the transcript from the Meetings page."

    static func displayMessage(preservedForRelaunch: Bool) -> String {
        if preservedForRelaunch {
            return "Transcripted couldn't safely queue that import. The copied audio was saved for retry on the Meetings page."
        }
